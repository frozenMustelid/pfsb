...
string PfMon::fetchMonNameTop() {

	string name;

	cout << "Name at the top of the monster entry, if different (press enter if same):\n";
	getline(cin,name);
...
string PfMon::fetchDescription() {

	cout << "Give a short description of the creature: ";
...
/* What makes a man turn neutral? Gold? Lust for
 * power? Or were you just born with a heart full
 * of neutrality?
 */
...
string PfMon::fetchSize() {
	...
	/*cout << "\t[1]  Aberration" << endl;
	cout << "\t[2]  Animal" << endl;
	cout << "\t[3]  Construct" << endl;
	cout << "\t[4]  Dragon" << endl;
	cout << "\t[5]  Fey" << endl;
	cout << "\t[6]  Humanoid" << endl;
	cout << "\t[7]  Magical Beast" << endl;
	cout << "\t[8]  Monstrous Humanoid" << endl;
	cout << "\t[9]  Ooze" << endl;
	cout << "\t[10] Outsider" << endl;
	cout << "\t[11] Plant" << endl;
	cout << "\t[12] Undead" << endl;
	cout << "\t[13] Vermin" << endl;//*/
...
string PfMon::determineSlas() {

	string choice;
	string frequency;
	int cl;
	int concentrationBonuses;
	...
	if (answerIsYes(choice)) {
		...
		cout << "Total concentration bonuses, except CON: ";
		cin >> concentrationBonuses;
		clearBuffer();

		concentration = (cl + abilities.conMod + concentrationBonuses);
		...
		slas = encloseInParen("CL " + cl + determineOrdinal(cl) + " Concentration +" + concentration) + slas;
	} else {
		slas = "None";
	}
...
string PfMon::addSlaListItem() {
	...
	do {
		cout << "This SLA's name: ";
		getline(cin,slaName);

		spellInfo = spellCanvas.determineSpell(abilities.intMod, slaName);
...
string PfMon::fetchSpecialAbilities() { //TODO: change to new encloseInElement function
